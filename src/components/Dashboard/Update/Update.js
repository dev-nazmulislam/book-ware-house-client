...
const Update = ({ book }) => {
  ...
  const handlePrice = (id) => {
    const updatBook = book;
    updatBook.price = newPrice;

    // send data to the server
    const url = `https://mighty-dusk-49836.herokuapp.com/bookprice/${id}`;
    fetch(url, {
      method: "PUT",
      headers: {
        "content-type": "application/json",
      },
      body: JSON.stringify(updatBook),
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleAddedBook = (id) => {
    ...
    if (quantity >= 1) {
      ...
      fetch(url, {
        ...
        .then((res) => res.json())
        .then((data) => {
          alert("Books Quantity added successfully!!!");
        });
